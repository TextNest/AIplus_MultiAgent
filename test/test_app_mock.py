...
# 1. 가상 데이터 정의
DUMMY_PLAN = "1. 데이터 기초 통계 확인\n2. 마케팅 채널별 성과 비교 및 시각화"
DUMMY_CODE = "import pandas as pd\nimport matplotlib.pyplot as plt\nprint('분석 수행 중...')\n# 가상 그래프 생성 시뮬레이션\nplt.figure(); plt.plot([1,2,3],[4,5,6]); plt.savefig('img/figure_0_0.png')"
DUMMY_REPORT = "# 🤖 가상 분석 보고서\n\n분석이 성공적으로 완료되었습니다.\n\n## 핵심 인사이트\n- 테스트용 가상 데이터 분석 결과입니다.\n- API 키 없이 흐름이 정상 작동함을 확인했습니다."

# 2. Mock LLM 동작 설정: 아주 단순한 순수 파이썬 클래스 정의 (Lock이 없어 피클링 안전)
class FakeResponse:
...
class FakeLLM:
    def invoke(self, prompt, config=None):
        p = str(prompt)
        # 검증(Evaluation) 프롬프트 대응: 반드시 'APPROVE'가 포함되어야 루프가 종료됩니다.
        if "APPROVE" in p or "검증" in p: content = "APPROVE"
        # 코드 생성 프롬프트 대응
        elif "코드를 작성" in p or "Python" in p: content = DUMMY_CODE
        # 계획 수립 프롬프트 대응
        elif "계획을 세우세요" in p: content = DUMMY_PLAN
        # 기타 (최종 보고서 등)
        else: content = DUMMY_REPORT
        return FakeResponse(content)
        # 프로젝트 내 실제 클래스 임포트 (피클링 안전)
        # 스크립트 내에 직접 정의(class ...)한 클래스는 픽클링 시 충돌을 일으킵니다.
    def with_structured_output(self, schema):        
        from src.Orc_agent.Node.sub_node.analyze_data import InsightOutput, ImageInsight
        # 내부 함수 정의
        def mock_struct_invoke(*args, **kwargs):
            return InsightOutput(
                overall_insight="분석 결과가 매우 긍정적입니다.",
                image_specific_insights=[ImageInsight(img_name="figure_0_0.png", insight="차트 분석 완료")]
            )
        # invoke 메서드를 가진 가짜 객체를 생성하여 반환 (들여쓰기 주의!)
        struct_obj = type('StructMock', (), {'invoke': mock_struct_invoke})
        return struct_obj()


# 2. [핵심] 체크포인트 저장 시 피클링(Serialization) 기능을 비활성화
# 스트림릿 UI 객체가 유입되어도 에러가 나지 않도록 메모리 저장 방식을 강제로 바꿉니다.
import langgraph.checkpoint.memory
class NoPickleMemorySaver(langgraph.checkpoint.memory.MemorySaver):
    def put(self, config, checkpoint, metadata, new_versions):
        # 내부적인 pickle.dumps 과정을 건너뛰고 메모리에 직접 객체를 저장합니다.
        thread_id = config["configurable"]["thread_id"]
        checkpoint_id = checkpoint["id"]
        if thread_id not in self.storage: self.storage[thread_id] = {}
        self.storage[thread_id][checkpoint_id] = (checkpoint, metadata)
        return config

# 3. 모든 공장(Factory) 및 모듈 패치 적용
def mock_create(*args, **kwargs): return FakeLLM(), []
# LLM 생성기 교체
patch("src.Orc_agent.core.llm_factory.LLMFactory.create", side_effect=mock_create).start()
# 체크포인터 클래스 자체를 교체 (가장 강력한 방어)
patch("langgraph.checkpoint.memory.MemorySaver", NoPickleMemorySaver).start()

# 4. [보조] observe 모듈의 deepcopy 방어
try:
    from src.Orc_agent.core import observe
    observe.merge_runnable_config = lambda base, **kwargs: (base or {}).copy()
except: pass
# 환경 변수 초기화 (Langfuse 등 외부 통신 차단)
os.environ["LANGFUSE_PUBLIC_KEY"] = ""
os.environ["LANGFUSE_SECRET_KEY"] = ""

# --- [END OF MOCKING LOGIC] ---
