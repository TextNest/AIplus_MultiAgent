...
def generate_content(state: ReportState) -> dict:
    ...
    if isinstance(analysis_results, dict):
        # 딕셔너리인 경우 (Orc_agent 스타일)
        # 예: {"summary": "요약내용", "details": "상세내용"}
        for key, value in analysis_results.items():
            if isinstance(value, str):
                content_parts.append(f"## {key.capitalize()}\n{value}")
    elif isinstance(analysis_results, list):
        # 리스트 스타일 호환성
        content_parts = [str(r) for r in analysis_results]
    else:
        content_parts = [str(analysis_results)]
...
def create_pptx(state: ReportState) -> dict:
    ...
    try:
        ...
        if analysis_results:
            text_content = ""
            if isinstance(analysis_results, dict):
                # 'summary' 키를 우선 찾고, 없으면 'result_summary'나 전체 값 사용
                text_content = analysis_results.get("summary") or analysis_results.get("result_summary") or str(analysis_results)
            elif isinstance(analysis_results, list) and analysis_results:
                text_content = analysis_results[0]
            ...
            text_content = str(text_content).replace("```python", "").replace("```", "")
